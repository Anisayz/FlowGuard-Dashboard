...
const RuleHistory: React.FC<RuleHistoryProps> = ({ rules }) => {
  if (rules.length === 0)
    return <p style={{ color: '#8888aa', textAlign: 'center', padding: '40px', fontFamily: 'monospace' }}>No history records.</p>;
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', fontFamily: 'monospace' }}>
        <thead>
          <tr style={{ background: '#14141e' }}>
            {['ID','Source IP','Action','Switch','Source','Created','Deleted','Alert ID'].map(h => (
